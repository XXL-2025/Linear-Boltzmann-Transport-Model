...
double max_e2XX = 15.0; //xx-25-12-14
double min_e2XX = 0.0;
double bin_e2XX = (max_e2XX - min_e2XX) / N_e2;
...
//...........................................tau-eta coordinates		  
		  ...
		  vc0[2]=VY00;
		  vc0[3]=VZ00;		  
		  if (abs(KATT10[i]) == 4) { alphas = alphas0(Kalphas, temp0); } //xx-25-12-14
		  qhat00=DebyeMass2(Kqhat0,alphas,temp00);		  
...
//...........................................t-z coordinates	
		  ...
		  if (i == 1) {
			  cout << "energy:" << P[0][i] << endl;
			  cout << "position:" << V[1][1] << "  " << V[2][1] << "  " << V[3][1] << endl;
			  cout << "momentum:" << P[1][1] << "  " << P[2][1] << "  " << P[3][1] << endl;
			  cout << "mass:" << sqrt(P[0][i] * P[0][i] - P[1][1] * P[1][1] - P[2][1] * P[2][1] - P[3][1] * P[3][1]) << endl;
		  }
//          //....test
//          if ( i == 21) {
...
//          // ... test
		  ...
		  cout << "bulklinear:" << endl;
		  cout << tau << "  " << xxx << "  " << yyy << "  " << eta << "  " << ed << "  " << temp0 << "  " << fraction << "  " << VX << "  " << VY << "  " << VZ << endl;
		  cout << "bulklinear_end:" << endl;
	

	//	  cout << " bulklinear:" << "tcar" << "  " << "xcar" << "ycar" << "  " << "zcar" << "  " << "temp0" << "  " << tau << "  " << xxx << yyy << "  " << eta << "  " << temp0 << endl;
      ...
      VZ=Vz_cal(eta,VZ);
           ...
           //cout<<"tau"<<" "<<tau<<" "<<"xxx"<<" "<<xxx<<" "<<"yyy"<<" "<<yyy<<" "<<"eta"<<" "<<eta<<" "<<endl;
		  
		  //cout<<"temp0"<<" "<<temp0<<" "<<"fraction_0"<<" "<<fraction<<" "<<"VX"<<" "<<VX<<" "<<"VY"<<" "<<VY<<" "<<"VZ"<<" "<<VZ<<endl;
...
//          }
		  ...
		  if (abs(KATT1[i]) == 4) { alphas = alphas0(Kalphas, temp0);  }//xx-25-12-14
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////...new		  
		  ...
		  if (abs(KATT1[i]) == 4) {//xx-25-12-14
			  ...
			  KPfactor = 1.0 + KPampXX * exp(-PLen * PLen / 2.0 / KPsig / KPsig);
			  KTfactor = 1.0 + KTampXX * exp(-pow((temp0 - hydro_Tc), 2) / 2.0 / KTsig / KTsig);
...
//........get scattering number in the current step
		  ...
		  cout << "KPampXX:" << KPampXX << "  " << "PLen:" << PLen << "  " << "KPsig:" << KPsig << endl;
		  cout << "KTampXX:" << KTampXX << "  " << "temp0:" << temp0 << "  " << "hydro_Tc:" << hydro_Tc << "  " << "KTsig:" << KTsig << endl;
		  cout << "KPfactor:" << KPfactor << "  " << "preKTXX:" << preKTXX << endl;
...
// get qhat from table????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
                  if(KATTC0==21) {
                      ...
                      RTE1=(qhatHQXX[iT2][iE1]-qhatHQXX[iT1][iE1])*(T-T1)/(T2-T1)+qhatHQXX[iT1][iE1];
...
//........time step in the local rest frame ???
          ...
          Tdiff=Tint_lrf[i];
		  cout << "ID:" << KATT1[i] << "  " << "Tint_lrf[i]:" << Tint_lrf[i] << "  " << "dt_lrf:" << dt_lrf << endl;
          //.. test
...
//.......get radiation number in the current step end		  	
		  ...
		  if (Kradiation == 1)
		  {
			  ...
			  if (abs(KATT1[i]) == 4) {//xx-25-12-14
				  ...
				  probCol = probCol * preKTXX * runKT + fraction * dt_lrf * striRTE / 0.1970;
				  probCol = (1.0 - exp(-probCol)) * (1.0 - probRad);
				  probTot = probCol + probRad;
				  cout << "probCol:" << probCol << "  " << "probRad:" << probRad << endl;
		  ...
		  else if (Kradiation == 0 && abs(KATT1[i]) == 4)
		  {
			  Reactionrate = probNcoll22 * (1.0 - probNrad) + probNrad;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////...new			  
			  ...
			  else { flavor(CT, KATTC0, KATT2, KATT3, RTE, PLen, T, T1, T2, E1, E2, iT1, iT2, iE1, iE2); }
		
			  cout << "flavor(CT, KATTC0, KATT2, KATT3, RTE, PLen, T, T1, T2, E1, E2, iT1, iT2, iE1, iE2):" << endl;
			  cout << CT << "  " << KATTC0 << "  " << KATT2 << "  " << KATT3 << "  " << RTE << "  " << PLen << "  " << T << "  " << T1 << "  " << T2 << "  " << E1 << "  " << E2 << "  " << iT1 << "  " << iT2 << "  " << iE1 << "  " << iE2 << endl;
			  cout << "flavor_end" << endl;
			  if (CT == 11) { // xx-10-21
				  collHQ22Qq(CT, temp0, qhat0, vc0, pc0, pc2, pc3, pc4, qt);

			  }
			  else if (CT == 12) {  // xx-10-21
			  ...
			  cout << "2-2" << endl;
			  cout << "CT,temp0,qhat0,vc0,pc0,pc2,pc3,pc4,qt:" << "  " << CT << "  " << temp0 << "  " << qhat0 << "  " << vc0[0] << "  " << pc0[0] << "  " << pc2[0] << "  " << pc3[0] << "  " << pc4[0] << "  " << qt << endl;
				  cout << "2-2_end" << endl;
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////...new
...
//			    if(Ejp>2*sqrt(qhat0) && ran0(&NUM1)<probNrad/Reactionrate && Kradiation==1 && qt!=0) Krad=1;
			  ...
			  ElabXX = pc4[0];
			  cout << "ElabXX:" << ElabXX << endl;
			  cout << "vc0:" << vc0[0] << "  " << vc0[1] << "  " << vc0[2] << "  " << vc0[3] << endl;
			  trans(vc0, pc4);
			  cout << "pc4:" << pc4[0] << "  " << pc4[1] << "  " << pc4[2] << "  " << pc4[3] << endl;
			  EjpXX = pc4[0];
			  transback(vc0, pc4);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////...new							  
							cout << "pc01[0]:" << pc01[0] << endl;
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////...new							  


							cout << "2-3" << endl;
							cout << KATT1[i] << "  " << temp0 << "  " << qhat0 << "  "  << vc0[1] << "  " << vc0[2] << "  " << vc0[3] << "  " << endl;
							cout << pc01[0] << "  " << pc01[1] << "  " << pc01[2] << "  " << pc01[3] << "  " << endl;
							cout << pc2[0] << "  " << pc2[1] << "  " << pc2[2] << "  " << pc2[3] << "  " << endl;
							cout << pc3[0] << "  " << pc3[1] << "  " << pc3[2] << "  " << pc3[3] << "  " << endl;
							cout << pc4[0] << "  " << pc4[1] << "  " << pc4[2] << "  " << pc4[3] << "  " << endl;
							cout << qt << "  " << icl23 << "  " << Tdiff << "  " << Ejp << "  " << endl;
							cout << maxFncHQ << "  " << lim_low << "  " << lim_int << endl;
							cout << "2-3_end" << endl;
...
//............test
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////...new
									  ...
									  radiationHQXX(KATT1[i], qhat0, vc0, pc2, pc01, pc4, pb, iclrad, Tdiff, Ejp, maxFncHQ, temp0, lim_low, lim_int);
								  }
								  ...
								  cout << "radiationHQ:" << endl;
								  cout << KATT1[i] << "  " << qhat0 << "  " << vc0[1] << "  " << vc0[2] << "  " << vc0[3] << "  " << endl;
								  cout << pc01[0] << "  " << pc01[1] << "  " << pc01[2] << "  " << pc01[3] << "  " << endl;
								  cout << pc2[0] << "  " << pc2[1] << "  " << pc2[2] << "  " << pc2[3] << "  " << endl;
								  cout << pc4[0] << "  " << pc4[1] << "  " << pc4[2] << "  " << pc4[3] << "  " << endl;
								  cout << pb << "  " << iclrad << "  " << Tdiff << "  " << Ejp << "  " << endl;
								  cout << maxFncHQ << "  " << lim_low << "  " << lim_int << "  " << temp0 << endl;
								  cout << "radiationHQ_end" << endl;
...
//				{
//				  CAT[i]=1;
//				  ncut=ncut+1;V[0][i]=-log(1.0-ran0(&NUM1));
////				  for(int j=0;j<=3;j++)
////					{
////					  PP[j][ncut]=P[j][i];
////					  VV[j][ncut]=V[j][i];
...
void LBTclass::lamXX(int KATT0, double& RTE, double& striRTE, double E, double T, double& T1, double& T2, double& E1, double& E2, int& iT1, int& iT2, int& iE1, int& iE2) {
	...
	if (KATT0 == 21) {
		...
		double RTE1 = (RHQXX[iT2][iE1] - RHQXX[iT1][iE1]) * (T - T1) / (T2 - T1) + RHQXX[iT1][iE1];
		double RTE2 = (RHQXX[iT2][iE2] - RHQXX[iT1][iE2]) * (T - T1) / (T2 - T1) + RHQXX[iT1][iE2];
		RTE = (RTE2 - RTE1) * (E - E1) / (E2 - E1) + RTE1;

		RTE1 = (striRHQ[iT2][iE1] - striRHQ[iT1][iE1]) * (T - T1) / (T2 - T1) + striRHQ[iT1][iE1];
...
void LBTclass::flavorXX(int& CT, int& KATT0, int& KATT2, int& KATT3, double RTE, double E, double T, double& T1, double& T2, double& E1, double& E2, int& iT1, int& iT2, int& iE1, int& iE2) {
	...
	//.....
	linearXX(KATT0, E, T, T1, T2, E1, E2, iT1, iT2, iE1, iE2, RTEg, RTEg1, RTEg2, RTEg3, RTEq, RTEq3, RTEq4, RTEq5, RTEq6, RTEq7, RTEq8, RTEHQ, RTEHQ11, RTEHQ12, qhatTP);
	...
	if (KATT00 == 21) {   //.....for gluon
		...
		double striRTEHQ11 = (RTE2 - RTE1) * (E - E1) / (E2 - E1) + RTE1;

		RTE1 = (striRHQ12[iT2][iE1] - striRHQ12[iT1][iE1]) * (T - T1) / (T2 - T1) + striRHQ12[iT1][iE1];
		RTE2 = (striRHQ12[iT2][iE2] - striRHQ12[iT1][iE2]) * (T - T1) / (T2 - T1) + striRHQ12[iT1][iE2];
		double striRTEHQ12 = (RTE2 - RTE1) * (E - E1) / (E2 - E1) + RTE1;

		double R0 = RTE;
		double R1 = RTEHQ11 * preKTXX * runKT + striRTEHQ11;
		double R2 = RTEHQ12 * preKTXX * runKT + striRTEHQ12;
		cout << "flavorXX-preKTXX:" << preKTXX << endl;
		double a = ran0(&NUM1);
...
void LBTclass::linearXX(int KATT, double E, double T, double& T1, double& T2, double& E1, double& E2, int& iT1, int& iT2, int& iE1, int& iE2, double& RTEg, double& RTEg1, double& RTEg2, double& RTEg3, double& RTEq, double& RTEq3, double& RTEq4, double& RTEq5, double& RTEq6, double& RTEq7, double& RTEq8, double& RTEHQ, double& RTEHQ11, double& RTEHQ12, double& qhatTP) {
	if (KATT == 21) {
		...
		double RTE1 = (RHQ11XX[iT2][iE1] - RHQ11XX[iT1][iE1]) * (T - T1) / (T2 - T1) + RHQ11XX[iT1][iE1];
		double RTE2 = (RHQ11XX[iT2][iE2] - RHQ11XX[iT1][iE2]) * (T - T1) / (T2 - T1) + RHQ11XX[iT1][iE2];
		RTEHQ11 = (RTE2 - RTE1) * (E - E1) / (E2 - E1) + RTE1;

		RTE1 = (RHQ12XX[iT2][iE1] - RHQ12XX[iT1][iE1]) * (T - T1) / (T2 - T1) + RHQ12XX[iT1][iE1];
		RTE2 = (RHQ12XX[iT2][iE2] - RHQ12XX[iT1][iE2]) * (T - T1) / (T2 - T1) + RHQ12XX[iT1][iE2];
...
//
void LBTclass::bulklinearXX(double tau, double x, double y, double eta, double& temp, double& VX, double& VY, double& VZ, int& fraction)
{
...
//  cout << pow(VXhydro[itau2][ix2][iy2][ieta2], 2) + pow(VYhydro[itau2][ix2][iy2][ieta2], 2) + pow(VZhydro[itau2][ix2][iy2][ieta2], 2) << endl; 
 ...
 /////////////////////////////////////////////////////////////////////////////  
  ...
  ed = xs1*ys1*etas1*taus1*edhydro[itau1][ix1][iy1][ieta1];
  ed+= xs1*ys1*etas1*taus2*edhydro[itau2][ix1][iy1][ieta1];
  ed+= xs1*ys1*etas2*taus1*edhydro[itau1][ix1][iy1][ieta2];
  ed+= xs1*ys1*etas2*taus2*edhydro[itau2][ix1][iy1][ieta2];
  ed+= xs1*ys2*etas1*taus1*edhydro[itau1][ix1][iy2][ieta1];
  ed+= xs1*ys2*etas1*taus2*edhydro[itau2][ix1][iy2][ieta1];
  ed+= xs1*ys2*etas2*taus1*edhydro[itau1][ix1][iy2][ieta2];
  ed+= xs1*ys2*etas2*taus2*edhydro[itau2][ix1][iy2][ieta2];
  ed+= xs2*ys1*etas1*taus1*edhydro[itau1][ix2][iy1][ieta1];
  ed+= xs2*ys1*etas1*taus2*edhydro[itau2][ix2][iy1][ieta1];
  ed+= xs2*ys1*etas2*taus1*edhydro[itau1][ix2][iy1][ieta2];
  ed+= xs2*ys1*etas2*taus2*edhydro[itau2][ix2][iy1][ieta2];
  ed+= xs2*ys2*etas1*taus1*edhydro[itau1][ix2][iy2][ieta1];
  ed+= xs2*ys2*etas1*taus2*edhydro[itau2][ix2][iy2][ieta1];
  ed+= xs2*ys2*etas2*taus1*edhydro[itau1][ix2][iy2][ieta2];
  ed+= xs2*ys2*etas2*taus2*edhydro[itau2][ix2][iy2][ieta2];
  
  }

  if(temp0medium<=0.1)   fraction=0.0;  
...
void LBTclass::radiationHQXX(int parID, double qhat0ud, double v0[4], double P2[4], double P3[4], double P4[4], double Pj0[4], int& ic, double Tdiff, double HQenergy, double max_Ng, double temp_med, double xLow, double xInt) {
...
double LBTclass::Mqc2qc_prime(double s, double t, double M, double temp) {
	...
	double q1, q2;
	double y1, y2, y3, y4;
	double aa, bb, A2qTprime, A2qT;
	double A1qT;
	double Aq;

	md = a_md + b_md * temp;

	MMC = 64.0 / 9.0 * (pow((m2m - u), 2) + pow((s - m2m), 2) + 2.0 * m2m * t) / (t - md * md) / (t - md * md) * alphafactor;

	mdprime = a_md + b_md * temp;
	msprime = sqrt(as_ms + bs_ms * temp);

	q = sqrt(-t);
	A1qT = (-16.0 * pi * alphasprime / 3.0) / (pow(mdprime, 2.0) - t);
	A2qT = (8.0 * pi * sigmaprime) / (msprime * msprime - t) / (msprime * msprime - t);
	MMS = (-t) * (4.0 * m2m - t) * pow(A2qT, 2.0) / (pow(Nc, 2.0) - 1);
	MMS = scaleSigma * MMS;  ////added temporarily
...
double LBTclass::Mgc2gc_prime(double s, double t, double M, double temp) {
	...
	MMC = 64.0 / 9.0 * ((s - m2m) * (m2m - u) + 2.0 * m2m * (s + m2m)) / ((s - m2m) * (s - m2m)) * alphafactor;
	MMC = MMC + 8.0 * (5 * m2m * m2m + 3 * m2m * t - 10 * m2m * u + 4 * t * t + 5 * t * u + 5 * u * u + (m2m - s) * (m2m - u)) / (t - md * md) / (t - md * md) * alphafactor;
	MMC = MMC + 64.0 / 9.0 * ((s - m2m) * (m2m - u) + 2.0 * m2m * (m2m + u)) / ((m2m - u) * (m2m - u)) * alphafactor;
	MMC = MMC + 16.0 * (3.0 * m2m * m2m - 3.0 * m2m * s - m2m * u + s * s) / ((s - m2m) * (t - md * md)) * alphafactor;
	MMC = MMC + 16.0 / 9.0 * (m2m * (4.0 * m2m - t)) / ((s - m2m) * (m2m - u)) * alphafactor;
	MMC = MMC + 16.0 * (3.0 * m2m * m2m - m2m * s - 3.0 * m2m * u + u * u) / ((t - md * md) * (u - m2m)) * alphafactor;
	...
	MMS = (-t) * (4.0 * m2m - t) * pow(A2qT, 2.0) * (CA / CF) / (pow(Nc, 2.0) - 1);
	MMS = scaleSigma * MMS;  ////added temporarily
	MM = MMC + MMS;
...
void LBTclass::collHQ22Qq(int CT, double temp, double qhat0ud, double v0[4], double p0[4], double p2[4], double p3[4], double p4[4], double& qt) {
	//
	//    HQ 2->2 scatterings
	//    p0 initial HQ momentum, output to final momentum
	//    p2 final thermal momentum, p3 initial thermal energy
	...
	double e1, e4, p1, cosTheta24, downFactor, sigFactor; // other useful variables
	double HQmass, fBmax, fFmax, fB, fF, maxValue;
	int index_p1XX, index_TXX, index_e2;
	int ct1_loop, ct2_loop, flag1, flag2;
	...
	index_p1XX = (int)((p1 - min_p1XX) / bin_p1XX);
	index_TXX = (int)((temp - min_TXX) / bin_TXX);
//	cout << "p1:" << p1 << "  " << "temp:" << temp << endl;
	//cout << "min_p1XX:" << min_p1XX << "  " << "bin_p1XX:" << bin_p1XX << endl;
	//cout << "index_p1XX:" << index_p1XX << "  " << "index_TXX:" << index_TXX << endl;
	if (index_p1XX >= N_p1XX) {
		index_p1XX = N_p1XX - 1;
		cout << "warning: p1 is over p_max: " << p1 << endl;
	}
	if (index_TXX >= N_TXX) {
	...
	runalphas = 4.0 * pi / 9.0 / log(2.0 * p0[0] * temp / 0.04);
	alphafactor = runalphas * fixAlphas * pi * pi;
	aa = 1.0 / (256.0 * pi * pi * pi * pi * p0[0]);

	fFmax = distFncFMXX[index_TXX][index_p1XX] * alphafactor + stridistFncFM[index_TXX][index_p1XX];  // maximum of f(xw) at given p1 and T
	...
	do {   // sample p2 (light parton) using distribution integrated over 3 angles
		...
		xw = max_e2XX * ran0(&NUM1);
		index_e2 = (int)((xw - min_e2XX) / bin_e2XX);
		if (index_e2 >= N_e2) index_e2 = N_e2 - 1;
		if (CT == 11) { // qc->qc
			ff = distFncFXX[index_TXX][index_p1XX][index_e2] * alphafactor + stridistFncF[index_TXX][index_p1XX][index_e2];
			ff = ff / fFmax;
			//cout << "Qq-ff:" << ff << endl;
			maxValue = distMaxFXX[index_TXX][index_p1XX][index_e2] * alphafactor + stridistMaxF[index_TXX][index_p1XX][index_e2];

		}
		else {
			cout << "Not Qq -> Qq process!" << endl;
			exit(EXIT_FAILURE);
		}
	} while (ran0(&NUM1) > ff);
//	cout << "max_e2XX:" << max_e2XX << "  " << "xw:" << xw << "  " << "temp:" << temp << endl;
	e2 = xw * temp;
	e1 = p0[0];

	// now e2 is fixed, need to sample the remaining 3 variables
	ct2_loop = 0;
	int test_22 = 0;//xx-12-9
	do {
		...
		// re-sample if the kinematic cuts are not satisfied
		if (ss <= (HQmass + md) * (HQmass + md) || (qperp2 >= (10.0 * temp) * (10.0 * temp))) {
			/////cccc          if( (qperp2>=(10.0*temp)*(10.0*temp)) ) {
			//xx-12-9

			if (test_22 == 1e6) {
				cout << "test_22" << "  " << "ss:" << ss << "  " << "(HQmass + md) * (HQmass + md):" << (HQmass + md) * (HQmass + md) << "  " << "qperp2:" << qperp2 << "  " << "(10.0 * temp) * (10.0 * temp):" << (10.0 * temp) * (10.0 * temp) << endl;
			}
			//xx-12-9
			rank = ran0(&NUM1);
			sigFactor = 0.0;
		...
		//cout << "ssRIGHT-Qq" << endl;
		if (CT == 11) {  // qc->qc
			ff = (1.0 / (exp(e2 / temp) + 1.0)) * (1.0 - 1.0 / (exp(e4 / temp) + 1.0));
			sigFactor = sigFactor * ff;
			///ccc 	       msq=Mqc2qc(ss,tt,HQmass)/maxValue;
			msq = Mqc2qc_prime(ss, tt, HQmass, temp) / maxValue;
			//	if (msq > 1) { cout << "Qq-msqerror" << endl; }
			//	cout <<"Mqc2qc_prime-Qq"<< Mqc2qc_prime(ss, tt, HQmass, temp) <<"  " << "MAXVALUE-Qq:" << maxValue << endl;
		}
		else {
			cout << "Not Qq -> Qq process!" << endl;
			exit(EXIT_FAILURE);
		...
		rank = ran0(&NUM1);
		//	cout << "msq*sigFactor-Qq:" << msq * sigFactor << endl;
	} while (rank > (msq * sigFactor));
...
void LBTclass::collHQ22Qg(int CT, double temp, double qhat0ud, double v0[4], double p0[4], double p2[4], double p3[4], double p4[4], double& qt) {
	//
	//    HQ 2->2 scatterings
	//    p0 initial HQ momentum, output to final momentum
	//    p2 final thermal momentum, p3 initial thermal energy
	...
	fBmax = distFncBMXX[index_TXX][index_p1XX] * alphafactor + stridistFncBM[index_TXX][index_p1XX];
	...
	do {   // sample p2 (light parton) using distribution integrated over 3 angles
		...
		if (index_e2 >= N_e2) index_e2 = N_e2 - 1;
		if (CT == 12) { // gc->gc
			ff = distFncBXX[index_TXX][index_p1XX][index_e2] * alphafactor + stridistFncB[index_TXX][index_p1XX][index_e2];
			ff = ff / fBmax;
			//cout << "Qg-ff:" << ff << endl;
			maxValue = distMaxBXX[index_TXX][index_p1XX][index_e2] * alphafactor + stridistMaxB[index_TXX][index_p1XX][index_e2];
		}
		else {
			cout << "Not Qg -> Qg process!" << endl;
			exit(EXIT_FAILURE);
		}
	} while (ran0(&NUM1) > ff);
	cout << "ct1_loop:" << ct1_loop << endl;
	cout << "max_e2XX:" << max_e2XX << "  " << "xw:" << xw << "  " << "temp:" << temp << endl;
	e2 = xw * temp;
	e1 = p0[0];
	// now e2 is fixed, need to sample the remaining 3 variables
	...
	do {
		...
		//	cout << "ssRIGHT-Qg" << endl;
		if (CT == 12) {  // gc->gc
			ff = (1.0 / (exp(e2 / temp) - 1.0)) * (1.0 + 1.0 / (exp(e4 / temp) - 1.0));
			sigFactor = sigFactor * ff;
			///ccc 	       msq=Mgc2gc(ss,tt,HQmass)/maxValue;
			msq = Mgc2gc_prime(ss, tt, HQmass, temp) / maxValue;
			//	if (msq > 1) { cout << "Qg-msqerror" << endl; }
			//	cout << "Mgc2gc_prime-Qq" << Mgc2gc_prime(ss, tt, HQmass, temp) << "  " << "MAXVALUE-Qg:" << maxValue << endl;
		}
		else {
			cout << "Not Qg -> Qg process!" << endl;
			exit(EXIT_FAILURE);
		...
		rank = ran0(&NUM1);
		//cout << "msq*sigFactor-Qg:" << msq * sigFactor << endl;
	} while (rank > (msq * sigFactor));
	cout << "ct2_loop:" << ct2_loop << endl;
...
void LBTclass::collHQ23XX(int parID, double temp_med, double qhat0ud, double v0[4], double p0[4], double p2[4], double p3[4], double p4[4], double qt, int& ic, double Tdiff, double HQenergy, double max_Ng, double xLow, double xInt) {
...
double LBTclass::nHQgluonXX(int parID, double dtLRF, double& time_gluon, double& temp_med, double& HQenergy, double& max_Ng) {
	// gluon radiation probability for heavy quark       
	...
	if (temp_med > temp_max) {
		...
		cout << time_gluon << "    " << temp_med << "    " << HQenergy << endl;
		//     temp_med=temp_max;
	}//xx-12-7   这里实际上应该修改，因为tempmax有两个，但是不影响计算
	...
	time_num = (int)(time_gluon / delta_tg + 0.5) + 1;   //xx-12-7 完全一致
	...
	//这个函数只有HF会使用，所以参数不同时直接使用HF专用变量即可
	...
	if (temp_num >= temp_gn) temp_num = temp_gn - 1;//25-12-7 还有一个temp_gnXX，不过数值相同，用哪个都一样
	...
	if (parID == 21) {
		...
		rate_T1E1 = dNg_over_dt_cXX[time_num][temp_num][HQenergy_num];
		rate_T1E2 = dNg_over_dt_cXX[time_num][temp_num][HQenergy_num + 1];
		rate_T2E1 = dNg_over_dt_cXX[time_num][temp_num + 1][HQenergy_num];
		rate_T2E2 = dNg_over_dt_cXX[time_num][temp_num + 1][HQenergy_num + 1];
		max_T1E1 = max_dNgfnc_cXX[time_num][temp_num][HQenergy_num];
		max_T1E2 = max_dNgfnc_cXX[time_num][temp_num][HQenergy_num + 1];
		max_T2E1 = max_dNgfnc_cXX[time_num][temp_num + 1][HQenergy_num];
	...
	rate_EGrid_low = rate_T1E1 + (temp_med - temp_min - temp_num * delta_tempXX) / delta_tempXX * (rate_T2E1 - rate_T1E1);
	rate_EGrid_high = rate_T1E2 + (temp_med - temp_min - temp_num * delta_tempXX) / delta_tempXX * (rate_T2E2 - rate_T1E2);
...
dNg_over_dt_cXX[i][j][k] = 0.0;
dNg_over_dt_qXX[i][j][k] = 0.0;
dNg_over_dt_gXX[i][j][k] = 0.0;
max_dNgfnc_cXX[i][j][k] = 0.0;
max_dNgfnc_qXX[i][j][k] = 0.0;
max_dNgfnc_gXX[i][j][k] = 0.0;
}
}
}
...
//...readin LBT input
   ...
   ifstream infile1("./file-bulk.in");
     infile1 >> readinhydro_route;
     infile1.close();
     
     cout<<"LBT->readinhydro_route"<<" "<<readinhydro_route<<endl;
     ...
     f0.getline(buf, 1024);
     cout << buf << endl;
     f0 >> dxh_r >> dyh_r >> detah_r >> dtauh_r;
     cout << dxh_r << "       " << dyh_r << "       " << detah_r << "       " << dtauh_r <<endl;
     f0.getline(buf, 1024);
     cout << buf << endl;

	  f0.getline(buf, 1024);
     cout << buf << endl;
     f0 >> x0h_r >> y0h_r >> eta0h_r >> tau0h_r;
     cout << x0h_r << "       " << y0h_r << "       " << eta0h_r << "       " << tau0h_r <<endl;
     f0.getline(buf, 1024);
     cout << buf << endl;
     f0.getline(buf, 1024);
     cout << buf << endl;
...
//     dtauhydro=0.3;	 
	 ...
	 //xx-25-12-14
	 char ratedataXX_route[1024];
	 sprintf(ratedataXX_route, "%s/HF/ratedata", rate_route);
	 ifstream f1XX(ratedataXX_route);
	 cout << ratedataXX_route << endl;
	 int na = 450;
	 if (!f1XX.is_open())
	 {
		 cout << "Erro openning date file1!\n";
	 ...
	 else
	 {
		 for (int i = 1; i <= na; i++)
		 {
			 f1XX >> it >> ie;
			 //f1>>Rg[it][ie]>>Rg1[it][ie]>>Rg2[it][ie]>>Rg3[it][ie]>>Rq[it][ie]>>Rq3[it][ie]>>Rq4[it][ie]>>Rq5[it][ie]>>Rq6[it][ie]>>Rq7[it][ie]>>Rq8[it][ie];
			 f1XX >> qhatGXX[it][ie] >> RgXX[it][ie] >> Rg1XX[it][ie] >> Rg2XX[it][ie] >> Rg3XX[it][ie] >> qhatLQXX[it][ie] >> RqXX[it][ie] >> Rq3XX[it][ie] >> Rq4XX[it][ie] >> Rq5XX[it][ie] >> Rq6XX[it][ie] >> Rq7XX[it][ie] >> Rq8XX[it][ie];
		 }
	 }
	 f1XX.close();
     ...
     sprintf(Hydroprofile_Tau_route,"%s/tau_end.dat",bulk3D_route);   
     ...
     tauend=tauhydroend; //511
	 
	 cout<<"tauhydroend"<<" "<<tauhydroend<<endl;
     ...
     //sprintf(Hydroprofile_bulk3D_route,"%s/bulk3D.dat",readinhydro_route);   	
...
//        f2.getline(buf, 1024);
	  ...
	  f2>>tauh>>edhydro[itau][ix][iy][ieta]>>temphydro[itau][ix][iy][ieta]>>fractionhydro[itau][ix][iy][ieta]>>VXhydro[itau][ix][iy][ieta]>>VYhydro[itau][ix][iy][ieta]>>VZhydro[itau][ix][iy][ieta];
...
//	  cout <<(itau - 1) * 0.3 + 0.2 << "  "  << temphydro[itau][1][1][1] << "  " <<fractionhydro[itau][1][1][1] << "   "<< VXhydro[itau][1][1][1] << "  " << VYhydro[itau][1][1][1] << "  " << VZhydro[itau][1][1][1] << endl;
	 ...
	 sprintf(ratedata_HQ_route, "%s/ratedata-HQ", rate_route);
	 ...
	 f11.close();
	 //xx-25-12-5
	 char ratedata_HQ_routeXX[1024];
	 sprintf(ratedata_HQ_routeXX, "%s/HF/ratedata-HQ", rate_route);
	 ifstream f11X(ratedata_HQ_routeXX);
	 cout << ratedata_HQ_routeXX << endl;
	 ...
	 else
	 {
		 ...
		 {
			 f11X >> it >> ie;
			 f11X >> RHQXX[it][ie] >> RHQ11XX[it][ie] >> RHQ12XX[it][ie] >> qhatHQXX[it][ie];
		 }
	 }
	 f11X.close();
	 ...
	 //xx-10-20 
	 char stri_ratedata_HQ_route[1024];
	 sprintf(stri_ratedata_HQ_route, "%s/HF/stri-ratedata-HQ", rate_route);
	 ifstream f11XX(stri_ratedata_HQ_route);
	 cout << stri_ratedata_HQ_route << endl;
	 if (!f11XX.is_open())
	 {
		 cout << "Erro openning stri-HQ data file!\n";
	 }
	 else
	 {
		 for (int i = 1; i <= na; i++)
		 {
			 f11XX >> it >> ie;
			 f11XX >> striRHQ[it][ie] >> striRHQ11[it][ie] >> striRHQ12[it][ie] >> striqhatHQ[it][ie];
			 striRHQ[it][ie] = scaleSigma * striRHQ[it][ie];
			 striRHQ11[it][ie] = scaleSigma * striRHQ11[it][ie];
			 striRHQ12[it][ie] = scaleSigma * striRHQ12[it][ie];
			 striqhatHQ[it][ie] = scaleSigma * striqhatHQ[it][ie];
		 }
	 }
	 f11XX.close();
	 //xx-10-20
   ...
   sprintf(dNg_over_dt_cD6_route,"%s/dNg_over_dt_cD6.dat",rate_route);   
...
//     if(!f12.is_open()||!f13.is_open()||!f14.is_open())
	 ...
	 char dNg_over_dt_cD6_routeXX[1024];
	 sprintf(dNg_over_dt_cD6_routeXX, "%s/HF/dNg_over_dt_cD6.dat", rate_route);
	 ifstream f12XX(dNg_over_dt_cD6_routeXX);
	 cout << dNg_over_dt_cD6_routeXX << endl;

	 char dNg_over_dt_qD6_routeXX[1024];
	 sprintf(dNg_over_dt_qD6_routeXX, "%s/HF/dNg_over_dt_qD6.dat", rate_route);
	 ifstream f13XX(dNg_over_dt_qD6_routeXX);
	 cout << dNg_over_dt_qD6_routeXX << endl;

	 char dNg_over_dt_gD6_routeXX[1024];
	 sprintf(dNg_over_dt_gD6_routeXX, "%s/HF/dNg_over_dt_gD6.dat", rate_route);
	 ifstream f14XX(dNg_over_dt_gD6_routeXX);
	 cout << dNg_over_dt_gD6_routeXX << endl;

	 //ifstream f12("readindatafile/dNg_over_dt_cD6.dat");
	 //ifstream f13("readindatafile/dNg_over_dt_qD6.dat");
	 //ifstream f14("readindatafile/dNg_over_dt_gD6.dat");
	 if (!f12XX.is_open() || !f13XX.is_open() || !f14XX.is_open())
	 {
		 cout << "Erro openning HQ radiation table file!\n";
	 }
	 else
	 {
		 for (int k = 1; k <= t_gn; k++) {
			 char dummyCharXX[100];
			 long double dummyDXX;
			 f12XX >> dummyCharXX >> dummyCharXX >> dummyCharXX >> dummyCharXX;
			 f13XX >> dummyCharXX >> dummyCharXX >> dummyCharXX >> dummyCharXX;
			 f14XX >> dummyCharXX >> dummyCharXX >> dummyCharXX >> dummyCharXX;
			 for (int i = 1; i <= temp_gnXX; i++) {
				 dNg_over_dt_cXX[k + 1][i][0] = 0.0;
				 dNg_over_dt_qXX[k + 1][i][0] = 0.0;
				 dNg_over_dt_gXX[k + 1][i][0] = 0.0;
				 max_dNgfnc_cXX[k + 1][i][0] = 0.0;
				 max_dNgfnc_qXX[k + 1][i][0] = 0.0;
				 max_dNgfnc_gXX[k + 1][i][0] = 0.0;
				 for (int j = 1; j <= HQener_gnXX; j++) {
					 f12XX >> dNg_over_dt_cXX[k + 1][i][j] >> max_dNgfnc_cXX[k + 1][i][j];
					 f13XX >> dNg_over_dt_qXX[k + 1][i][j] >> max_dNgfnc_qXX[k + 1][i][j];
					 f14XX >> dNg_over_dt_gXX[k + 1][i][j] >> max_dNgfnc_gXX[k + 1][i][j];
				 }
			 }
		 }
	 }
	 cout << dNg_over_dt_cXX[t_gn + 1][temp_gnXX][HQener_gnXX] << "    " << max_dNgfnc_cXX[t_gn + 1][temp_gnXX][HQener_gnXX] << endl;
	 cout << dNg_over_dt_qXX[t_gn + 1][temp_gnXX][HQener_gnXX] << "    " << max_dNgfnc_qXX[t_gn + 1][temp_gnXX][HQener_gnXX] << endl;
	 cout << dNg_over_dt_gXX[t_gn + 1][temp_gnXX][HQener_gnXX] << "    " << max_dNgfnc_gXX[t_gn + 1][temp_gnXX][HQener_gnXX] << endl;
	 f12XX.close();
	 f13XX.close();
	 f14XX.close();
...
// preparation for HQ 2->2
	 ...
	 char distB_routeXX[1024];
	 sprintf(distB_routeXX, "%s/HF/distB.dat", rate_route);
	 ifstream fileBXX(distB_routeXX);

	 cout << distB_routeXX << endl;
	 ...
	 if (!fileBXX.is_open()) {
		 ...
		 for (int i = 0; i < N_TXX; i++) {
			 for (int j = 0; j < N_p1XX; j++) {
				 double dummy_TXX, dummy_p1XX;
				 fileBXX >> dummy_TXX >> dummy_p1XX;
				 if (fabs(min_TXX + (0.5 + i) * bin_TXX - dummy_TXX) > 1.0e-5 || fabs(min_p1XX + (0.5 + j) * bin_p1XX - dummy_p1XX) > 1.0e-5) {

					 cout << "Erro in reading data file distB.dat!" << endl;
					 exit(EXIT_FAILURE);
				 }
				 fileBXX >> distFncBMXX[i][j];
				 for (int k = 0; k < N_e2; k++) fileBXX >> distFncBXX[i][j][k];
				 for (int k = 0; k < N_e2; k++) fileBXX >> distMaxBXX[i][j][k];
			 }
		 }
	 }
	 fileBXX.close();
	 ...
	 cout << "distB" << endl;
	 cout << distFncBXX[3][3][3] << endl;
	 cout << distFncBXX[2][4][4] << endl;
	 cout << distFncBXX[5][4][5] << endl;
	 cout << distMaxBXX[3][1][3] << endl;
	 cout << distMaxBXX[7][4][4] << endl;
	 cout << distMaxBXX[5][5][7] << endl;




	 char test[1024];
	 sprintf(test, "%s/HF/stri-distB.dat", rate_route);
	 ifstream testfile(test);
	 cout << test << endl;
	 if (!testfile.is_open()) {
		 cout << "Erro openning data file stri-distB.dat!" << endl;
	 }
	 else {
		 for (int i = 0; i < N_TXX; i++) {
			 for (int j = 0; j < N_p1XX; j++) {
				 double dummy_T, dummy_p1;
				 testfile >> dummy_T >> dummy_p1;
				 //	 cout << "??????" << i << "  " << min_T << "  " << bin_T << "  " << min_p1 << "  " << bin_p1 << endl;
				 if (fabs(min_TXX + (0.5 + i) * bin_TXX - dummy_T) > 1.0e-5 || fabs(min_p1XX + (0.5 + j) * bin_p1XX - dummy_p1) > 1.0e-5) {
					 // cout << i << "  " <<j<<"  " << min_T << "  " << bin_T << "  " << min_p1 << "  " << bin_p1 << "  " << dummy_TXX << "  " << dummy_p1XX << endl;
					 // cout << fabs(min_T + (0.5 + i) * bin_T - dummy_TXX) << endl;
					 // cout << fabs(min_p1 + (0.5 + j) * bin_p1 - dummy_p1XX) << endl;
					 // cout << "Error in reading data file stri-distB.dat!" << endl;
					 exit(EXIT_FAILURE);
				 }
				 testfile >> stridistFncBM[i][j];
				 for (int k = 0; k < N_e2; k++) testfile >> stridistFncB[i][j][k];
				 for (int k = 0; k < N_e2; k++) testfile >> stridistMaxB[i][j][k];
				 stridistFncBM[i][j] = scaleSigma * stridistFncBM[i][j];
				 for (int k = 0; k < N_e2; k++) stridistFncB[i][j][k] = scaleSigma * stridistFncB[i][j][k];
				 for (int k = 0; k < N_e2; k++) stridistMaxB[i][j][k] = scaleSigma * stridistMaxB[i][j][k];
				 // if consider Yukawa interaction only;

			 }
		 }
	 }
	 testfile.close();
	 cout << "stri-distB" << endl;
	 cout << stridistFncB[3][3][3] << endl;
	 cout << stridistFncB[2][4][4] << endl;
	 cout << stridistFncB[5][4][5] << endl;
	 cout << stridistMaxB[3][1][3] << endl;
	 cout << stridistMaxB[7][4][4] << endl;
	 cout << stridistMaxB[5][5][7] << endl;
	 cout << stridistFncBM[7][5] << endl;


	 char stri_distF_route[1024];
	 sprintf(stri_distF_route, "%s/HF/stri-distF.dat", rate_route);
	 ifstream strifileF(stri_distF_route);
	 if (!strifileF.is_open()) {
		 cout << "Erro openning data file stri-distF.dat!" << endl;
	 }
	 else {
		 for (int i = 0; i < N_TXX; i++) {
			 for (int j = 0; j < N_p1XX; j++) {
				 double dummy_T, dummy_p1;
				 strifileF >> dummy_T >> dummy_p1;
				 if (fabs(min_TXX + (0.5 + i) * bin_TXX - dummy_T) > 1.0e-5 || fabs(min_p1XX + (0.5 + j) * bin_p1XX - dummy_p1) > 1.0e-5) {
					 cout << "Erro in reading data file stri-distF.dat!" << endl;
					 exit(EXIT_FAILURE);
				 }
				 strifileF >> stridistFncFM[i][j];
				 for (int k = 0; k < N_e2; k++) strifileF >> stridistFncF[i][j][k];
				 for (int k = 0; k < N_e2; k++) strifileF >> stridistMaxF[i][j][k];
				 stridistFncFM[i][j] = scaleSigma * stridistFncFM[i][j];
				 for (int k = 0; k < N_e2; k++) stridistFncF[i][j][k] = scaleSigma * stridistFncF[i][j][k];
				 for (int k = 0; k < N_e2; k++) stridistMaxF[i][j][k] = scaleSigma * stridistMaxF[i][j][k];
				 // if consider Yukawa interaction only;
	 ...
	 cout << "stri-distF" << endl;
	 cout << stridistFncF[3][3][3] << endl;
	 cout << stridistFncF[2][4][4] << endl;
	 cout << stridistFncF[5][4][5] << endl;
	 cout << stridistMaxF[3][1][3] << endl;
	 cout << stridistMaxF[7][4][4] << endl;
	 cout << stridistMaxF[5][5][7] << endl;
	 cout << stridistFncFM[7][5] << endl;
	 ...
	 char distF_routeXX[1024];
	 sprintf(distF_routeXX, "%s/HF/distF.dat", rate_route);
	 ifstream fileFXX(distF_routeXX);
	 cout << distF_routeXX << endl;
	 ...
	 if (!fileFXX.is_open()) {
		 ...
		 for (int i = 0; i < N_TXX; i++) {
			 for (int j = 0; j < N_p1XX; j++) {
				 double dummy_T, dummy_p1;
				 fileFXX >> dummy_T >> dummy_p1;
				 if (fabs(min_TXX + (0.5 + i) * bin_TXX - dummy_T) > 1.0e-5 || fabs(min_p1XX + (0.5 + j) * bin_p1XX - dummy_p1) > 1.0e-5) {
					 cout << "Erro in reading data file distF.dat!" << endl;
					 exit(EXIT_FAILURE);
				 }
				 fileFXX >> distFncFMXX[i][j];
				 for (int k = 0; k < N_e2; k++) fileFXX >> distFncFXX[i][j][k];
				 for (int k = 0; k < N_e2; k++) fileFXX >> distMaxFXX[i][j][k];
			 }
		 }
	 }
	 fileFXX.close();
	 ...
	 cout << "distF" << endl;
	 cout << distFncFXX[3][3][3] << endl;
	 cout << distFncFXX[2][4][4] << endl;
	 cout << distFncFXX[5][4][5] << endl;
		...
		for(int ix=0;ix<=200;ix++)
      		{
      			...
      			{
      				yyy_g=y0h_r+dy*iy;
      				bulklinear(tau_g,xxx_g,yyy_g,eta_g,ed_g,temp0_g,fraction_g,VX_g,VY_g,VZ_g);
      				//cout<<"tau"<<" "<<tau_g<<" "<<"xxx"<<" "<<xxx_g<<" "<<"yyy"<<" "<<yyy_g<<" "<<"eta"<<" "<<eta_g<<" "<<endl;
		  		//cout<<"xxx:"<<xxx_g<<" "<<"yyy:"<<yyy_g<<endl;		
		  		//cout<<"temp0"<<" "<<temp0<<" "<<"fraction_0"<<" "<<fraction<<" "<<"VX"<<" "<<VX<<" "<<"VY"<<" "<<VY<<" "<<"VZ"<<" "<<VZ<<endl;
		  		k_g++;
      				numjet[k_g]=ed_g;
      				Xnucleon[k_g]=xxx_g;
      				Ynucleon[k_g]=yyy_g;
      				numjettotal=numjettotal+numjet[k_g];
      			//	cout<<"k_g:"<<k_g<<" "<<"numjet[k_g]:"<<numjet[k_g]<<" "<<"Xnucleon[k_g]:"<<Xnucleon[k_g]<<" "<<"Ynucleon[k_g]:"<<Ynucleon[k_g]<<endl;
