...
int main(int argc, char **argv) {	
 ...
 //   cout << LBT->NUM1 << endl;
  ...
  ofstream negativeLBT("./outputdatafile/negative.dat");
    ...
    ofstream positiveInfo("positive_info.dat");//xx-26-1-10
...
//......Parton information read in
        ...
        for(unsigned ip=1; ip<=njetshower; ++ip)
        {
           ...
           // f4 >> ne >> fjetshower >> pxjetshower >> pyjetshower >> pzjetshower >> pythiap0 >> Aleading >> vxjetshower >> vyjetshower >> vzjetshower >> vtjetshower;	
			...
			p0jetshower=sqrt(pow(pxjetshower,2)+pow(pyjetshower,2)+pow(pzjetshower,2));     //current calculation with light partons assume that they are massless 
			...
			if(abs(fjetshower)!=21 && abs(fjetshower)!=1 && abs(fjetshower)!=2 && abs(fjetshower)!=3) continue;     //parton cut
