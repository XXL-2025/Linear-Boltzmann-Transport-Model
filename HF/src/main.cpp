...
//test-XX-25-11-24
 ...
 P[0][i] =100;
 P[4][i]=1.27;
 P[2][i]=sqrt(P[0][1]*P[0][1] - P[4][i] * P[4][i]);
Vfrozen[1][i]=0;
	 Vfrozen[2][i]=0;
	 Vfrozen[3][i]=0;
     Vfrozen[0][i] = 0.6;
//	 Vfrozen[0][i]=0.6;
	...
	//	cout<< "dummyInt:"<<dummyInt<<" "<<" KATT1[i]:"<<KATT1[i]<<" "<<"P[1][i]:"<<P[1][i]<<" "<<"P[2][i]:"<< P[2][i]<<" "<<"P[3][i]:"<<P[3][i]<<" "<<"P[0][i]:"<<P[0][i]<<" "<<"P[4][i]:"<<P[4][i]<<" "<< "Vfrozen[1][i]:"<<Vfrozen[1][i]<<" "<<"Vfrozen[2][i]:"<<Vfrozen[2][i]<<" "<<"Vfrozen[3][i]:"<<Vfrozen[3][i]<<" "<<"Vfrozen[0][i]:"<< Vfrozen[0][i]<<endl;
                V[1][i]=Vfrozen[1][i];
                V[2][i]=Vfrozen[2][i];
                V[3][i]=Vfrozen[3][i];
                V[0][i] = Vfrozen[0][i];
...
//...end initilization of jet parton
            //xx-25-12-17
        if (heavyOut == 1 && vacORmed == 1) {
            // 输出初始时刻数据
            for (int i = 1; i <= np; i++) {
                if (abs(KATT1[i]) == 4) { // 只输出重夸克
                    outHQ << "INIT " << n << " " << time0 << " " << KATT1[i] << "    "
                        << P[1][i] << "    " << P[2][i] << "    " << P[3][i] << "    "
                        << P[0][i] << "    " << P[4][i] << "    " << Vfrozen[1][i] << "    "
                        << Vfrozen[2][i] << "    " << Vfrozen[3][i] << "    " << Vfrozen[0][i] << "    "
                        << Tfrozen[i] << "    " << vcfrozen[1][i] << "    " << vcfrozen[2][i] << "    "
                        << vcfrozen[3][i] << "    " << P[5][i] << "    " << P[6][i] << endl;
                }
            }
        }
        //xx-25-12-17
...
//        cout<<"EfTot: "<<EfTot<<endl;
        ...
        if(outFormat==1){
          ...
          // xx-25-12-17 if(heavyOut==1) outHQ << n << "    " << iHQ << endl;
...
//                if(abs(KATT1[i])==4)
//		{ // write out heavy quark
//			if(heavyOut==1) 
//			{
//				if(KATT1[i]==4) 
//				{
//					outHQ << "5" << "    " << P[1][i] << "    " << P[2][i] << "    " << P[3][i] << "    " << P[0][i] << "    " << P[4][i] << "    " << Vfrozen[1][i] << "    " << Vfrozen[2][i] << "    " << Vfrozen[3][i] << "    " << Vfrozen[0][i] << "    " << Tfrozen[i] << "    " << vcfrozen[1][i] << "    " << vcfrozen[2][i] << "    " << vcfrozen[3][i] << "    " << P[5][i] << "    " << WT[i] << endl;
//				}
//				if(KATT1[i]==-4) 
//				{
//					outHQ << "-5" << "    " << P[1][i] << "    " << P[2][i] << "    " << P[3][i] << "    " << P[0][i] << "    " << P[4][i] << "    " << Vfrozen[1][i] << "    " << Vfrozen[2][i] << "    " << Vfrozen[3][i] << "    " << Vfrozen[0][i] << "    " << Tfrozen[i] << "    " << vcfrozen[1][i] << "    " << vcfrozen[2][i] << "    " << vcfrozen[3][i] << "    " << P[5][i] << "    " << WT[i] << endl;
...
//                }                 
		...
		if(abs(KATT1[i])==4){ // write out heavy quark
                 //xx-25-12-17   if(heavyOut==1) outHQ << KATT1[i] << "    " << P[1][i] << "    " << P[2][i] << "    " << P[3][i] << "    " << P[0][i] << "    " << P[4][i] << "    " << Vfrozen[1][i] << "    " << Vfrozen[2][i] << "    " << Vfrozen[3][i] << "    " << Vfrozen[0][i] << "    " << Tfrozen[i] << "    " << vcfrozen[1][i] << "    " << vcfrozen[2][i] << "    " << vcfrozen[3][i] << "    " << P[5][i] << "    " << P[6][i] << endl;
                        
        } 
		else { // write out light parton
                    if(lightOut==1) positive << KATT1[i] <<  "    " << P[1][i] << "    " << P[2][i] << "    " << P[3][i] << "    " << P[0][i] << "    " << P[4][i] << "    " << Vfrozen[1][i] << "    " << Vfrozen[2][i] << "    " << Vfrozen[3][i] << "    " << Vfrozen[0][i] << "    " << Tfrozen[i] << "    " << vcfrozen[1][i] << "    " << vcfrozen[2][i] << "    " << vcfrozen[3][i] << "    " << P[5][i] << "    " << WT[i] << endl;
                }
            }	
            ...
            for(int i=2;i<=np;i++) {
                ...
                if(lightOut==1) negative << KATT10[i] << "    " << P0[1][i] << "    " << P0[2][i] << "    " << P0[3][i] << "    " << P0[0][i] << "    " << P0[4][i] << "    " << Vfrozen0[1][i] << "    " << Vfrozen0[2][i] << "    " << Vfrozen0[3][i] << "    " << Vfrozen0[0][i] << "    " << Tfrozen0[i] << "    " << vcfrozen0[1][i] << "    " << vcfrozen0[2][i] << "    " << vcfrozen0[3][i] << "    " << P0[5][i] << "    " << WT0[i] << endl;
            }	
...
//	cout << "eGluon" << " " << eGluon << " " << "nGluon" << " " << nGluon << " " << endl;
