...
void read_tables(){ // intialize various tables for LBT
     ...
     */
     ...
     sprintf(Hydroprofile_bulk3D_route,"/home/hyj/lxx/LBT_MERGE_version2/input/Hydroprofile_5020_010_visc/bulk3D.dat");     	
...
//     ifstream f11("b-tables/ratedata-HQ"); 		//for b quark
     if(!f11.is_open())
     {
...
// if consider string interaction only;
	  if(flagPNP==1)
          {
	  RHQ[it][ie]=0.0;
	  RHQ11[it][ie]=0.0;
...
// duplicate for heavy quark
     ifstream strif11("tables/stri-ratedata-HQ"); 		//for c quark
     if(!strif11.is_open())
     {
     ...
     else
     {
	  ...
	  {
	  strif11>>it>>ie;
	  strif11>>striRHQ[it][ie]>>striRHQ11[it][ie]>>striRHQ12[it][ie]>>striqhatHQ[it][ie];
...
// if consider Yukawa interaction only;
	  if(flagPNP==0)
          { 
          striRHQ[it][ie]=0.0*striRHQ[it][ie];
          striRHQ11[it][ie]=0.0*striRHQ11[it][ie];
          striRHQ12[it][ie]=0.0*striRHQ12[it][ie];
          striqhatHQ[it][ie]=0.0*striqhatHQ[it][ie];
	  }
	  }
     }
     strif11.close();
...
//     ifstream fileB("b-tables/distB.dat");
	 ...
	 //    cout << "??????" << i << "  " << min_T << "  " << bin_T << "  " << min_p1 << "  " << bin_p1 << endl; 
              if(fabs(min_T+(0.5+i)*bin_T-dummy_T)>1.0e-5 || fabs(min_p1+(0.5+j)*bin_p1-dummy_p1)>1.0e-5) {
                  cout << "Error in reading data file distB.dat!" << endl;
                  exit (EXIT_FAILURE);
              }
              fileB>>distFncBM[i][j];
...
// if consider string interaction only;
              ...
              {
              distFncBM[i][j]=0.0;
              for(int k=0;k<N_e2;k++) distFncB[i][j][k]=0.0;
              for(int k=0;k<N_e2;k++) distMaxB[i][j][k]=0.0;
	      }
           }
        }
     }
     ...
     cout << "distB" << endl;
     cout << distFncB[3][3][3] << endl;
     cout << distFncB[2][4][4] << endl;
     cout << distFncB[5][4][5] << endl;
     cout << distMaxB[3][1][3] << endl;
     cout << distMaxB[7][4][4] << endl;
     cout << distMaxB[5][5][7] << endl;

     ifstream strifileB("tables/stri-distB.dat");
     if(!strifileB.is_open()) {
        cout << "Erro openning data file stri-distB.dat!" << endl;
     } else {
        for(int i=0;i<N_T;i++) {
           for(int j=0;j<N_p1;j++) {
              double dummy_T,dummy_p1;
              strifileB>>dummy_T>>dummy_p1;
	      //if(i==1){
	        // 	 cout << i << "  " <<j<<"  " << min_T << "  " << bin_T << "  " << min_p1 << "  " << bin_p1 << "  " << dummy_T << "  " << dummy_p1 << endl;
					
                  //                       cout << fabs(min_T + (0.5 + i) * bin_T - dummy_T) << endl;
                    //                     cout << fabs(min_p1 + (0.5 + j) * bin_p1 - dummy_p1) << endl;
	     // }
					 if(fabs(min_T+(0.5+i)*bin_T-dummy_T)>1.0e-5 || fabs(min_p1+(0.5+j)*bin_p1-dummy_p1)>1.0e-5) {
           	//				 cout << i << "  " << min_T << "  " << bin_T << "  " << min_p1 << "  " << bin_p1 << endl;
		//			 cout << fabs(min_T + (0.5 + i) * bin_T - dummy_T) << endl;
		//			 cout << fabs(min_p1 + (0.5 + j) * bin_p1 - dummy_p1) << endl;
	       	      cout << "Erro in reading data file stri-distB.dat!" << endl;
                  exit (EXIT_FAILURE);
              }
              strifileB>>stridistFncBM[i][j];
              for(int k=0;k<N_e2;k++) strifileB>>stridistFncB[i][j][k];
              for(int k=0;k<N_e2;k++) strifileB>>stridistMaxB[i][j][k];
...
// if consider Yukawa interaction only;
              if(flagPNP==0)
	      {
              stridistFncBM[i][j]=0.0*stridistFncBM[i][j];
              for(int k=0;k<N_e2;k++) stridistFncB[i][j][k]=0.0*stridistFncB[i][j][k];
              for(int k=0;k<N_e2;k++) stridistMaxB[i][j][k]=0.0*stridistMaxB[i][j][k];
	      }
           }
        }
     }
...
// if consider string interaction only;
              ...
              {
              distFncFM[i][j]=0.0;
              for(int k=0;k<N_e2;k++) distFncF[i][j][k]=0.0;
              for(int k=0;k<N_e2;k++) distMaxF[i][j][k]=0.0;
	      }
           }
        }
     }
     ...
     cout << "distF" << endl;
     cout << distFncF[3][3][3] << endl;
     cout << distFncF[2][4][4] << endl;
     cout << distFncF[5][4][5] << endl;
     cout << distMaxF[3][1][3] << endl;
     cout << distMaxF[7][4][4] << endl;
     cout << distMaxF[5][5][7] << endl;



     ifstream strifileF("tables/stri-distF.dat");
...
// if consider Yukawa interaction only;
              if(flagPNP==0)
	      {
              stridistFncFM[i][j]=0.0*stridistFncFM[i][j];
              for(int k=0;k<N_e2;k++) stridistFncF[i][j][k]=0.0*stridistFncF[i][j][k];
              for(int k=0;k<N_e2;k++) stridistMaxF[i][j][k]=0.0*stridistMaxF[i][j][k];
	      }
           }
        }
     }
...
void lam(int KATT0,double &RTE,double &striRTE,double E,double T,double &T1,double &T2,double &E1,double &E2,int &iT1,int &iT2,int &iE1,int &iE2){   
...
void flavor(int &CT,int &KATT0,int &KATT2,int &KATT3,double RTE,double E,double T,double &T1,double &T2,double &E1,double &E2,int &iT1,int &iT2,int &iE1,int &iE2){  
  ...
  if(KATT00==21) {   //.....for gluon
	  ...
	  double striRTEHQ12=(RTE2-RTE1)*(E-E1)/(E2-E1)+RTE1;	
          ...
          double R0=RTE;
	  double R1=RTEHQ11*preKT*runKT+striRTEHQ11;
	  double R2=RTEHQ12*preKT*runKT+striRTEHQ12;
...
//.........................................................................
void collHQ22Qq(int CT,double temp,double qhat0ud,double v0[4],double p0[4],double p2[4],double p3[4],double p4[4],double &qt){
  //
  //    HQ 2->2 scatterings
...
// continue this function for HQ scattering
  ...
  //cout << "min_p1:" << min_p1 << "  " << "bin_p1:" << bin_p1 << endl;
  //cout << "index_p1:" << index_p1 << "  " << "index_T:" << index_T << endl;
  if(index_p1>=N_p1) {
      index_p1=N_p1-1;
  ...
  fFmax=distFncFM[index_T][index_p1]*alphafactor+stridistFncFM[index_T][index_p1];  // maximum of f(xw) at given p1 and T
  ...
  do {   // sample p2 (light parton) using distribution integrated over 3 angles
        ...
        if(CT==11) { // qc->qc
            ff=distFncF[index_T][index_p1][index_e2]*alphafactor+stridistFncF[index_T][index_p1][index_e2];
            ff=ff/fFmax;
         //   cout << "ff:" << ff << endl;
            maxValue=distMaxF[index_T][index_p1][index_e2]*alphafactor+stridistMaxF[index_T][index_p1][index_e2];
        } else {
            cout << "Not Qq -> Qq process!" << endl;
            exit(EXIT_FAILURE);
        }
  } while(ran0(&NUM1)>ff);
  //cout << "max_e2:" << max_e2 << "  " << "xw:" << xw << "  " << "temp:" << temp << endl;
  e2=xw*temp;
  e1=p0[0];
  //cout << "e1:" << e1 << "  " << "e2:" << e2 << "  " << "HQmass:" << HQmass << "  " << "p1:" << p1 << "  " << "cos(theta2):" << cos(theta2) << endl;
...
///ccc 	       msq=Mqc2qc(ss,tt,HQmass)/maxValue;
  ...
  //cout << "Qq-ff:" << ff << "  " << "xw" << xw << "  " << "temp" << temp << "e2:" << e2 << "e4:" << e4 << endl;

  //cout << "downFactor:" << downFactor << endl;
  //cout << "msq-Qq:" << msq << "  " << "sigFactor:" << sigFactor << endl;
  //cout << "HQmass:" << HQmass << "  " << "qperp2:" << qperp2 << endl;
  if(flag1==0 && flag2==0) {
...
//.........................................................................
void collHQ22Qg(int CT,double temp,double qhat0ud,double v0[4],double p0[4],double p2[4],double p3[4],double p4[4],double &qt){
  //
  //    HQ 2->2 scatterings
  //    p0 initial HQ momentum, output to final momentum
  //    p2 final thermal momentum, p3 initial thermal energy
...
// continue this function for HQ scattering
  ...
  fBmax=distFncBM[index_T][index_p1]*alphafactor+stridistFncBM[index_T][index_p1];
  ...
  do {   // sample p2 (light parton) using distribution integrated over 3 angles
        ...
        if(index_e2>=N_e2) index_e2=N_e2-1;
        if(CT==12) { // gc->gc
            ff=distFncB[index_T][index_p1][index_e2]*alphafactor+stridistFncB[index_T][index_p1][index_e2];
            ff=ff/fBmax;
            maxValue=distMaxB[index_T][index_p1][index_e2]*alphafactor+stridistMaxB[index_T][index_p1][index_e2];
        } else {
...
///ccc 	       msq=Mgc2gc(ss,tt,HQmass)/maxValue;
 ...
 // cout << "Qg-ff:" << ff << "  " << "xw" << xw << "  " << "temp" << temp << "e2:" << e2 << "e4:" << e4 << endl;
 // cout << "downFactor:" << downFactor << endl;
 // cout << "msq-Qg:" << msq << "  " << "sigFactor:" << sigFactor << endl;
 // cout << "pp3mag:" << pp3mag << "  " << "p1dotp3:" << p1dotp3 << "  " << "p1:" << p1 << endl;
 // cout << "HQmass:" << HQmass << "  " << "qperp2:" << qperp2 << endl;
  if(flag1==0 && flag2==0) {
      ...
      cout << "mass9:" << sqrt(p0[0] * p0[0] - p0[1] * p0[1] - p0[2] * p0[2] - p0[3] * p0[3]) << endl;
      // calculate qt in the rest frame of medium
      rotate(p4[1],p4[2],p4[3],p0,1);
      qt=sqrt(pow(p0[1],2)+pow(p0[2],2));
      rotate(p4[1],p4[2],p4[3],p0,-1);
      cout << "mass10:" << sqrt(p0[0] * p0[0] - p0[1] * p0[1] - p0[2] * p0[2] - p0[3] * p0[3]) << endl;
      // transform from comoving frame to the lab frame
      transback(v0,p2);
      transback(v0,p0);
      transback(v0,p3);
      transback(v0,p4);
      cout << "mass5:" << sqrt(p0[0] * p0[0] - p0[1] * p0[1] - p0[2] * p0[2] - p0[3] * p0[3]) << endl;
  } else { // no scattering
      transback(v0,p0);
      transback(v0,p4);
      qt=0;
...
//  HQenergy_num=(int)(HQenergy/delta_HQener+0.5); // use linear interpolation instead of finding nearest point for E and T dimensions
  ...
  cout << "temp_med:" << temp_med << "  " << "temp_min:" << temp_min << "  " << "delta_temp:" << delta_temp << "  " << "temp_num:" << temp_num << endl;
  HQenergy_num=(int)(HQenergy/delta_HQener); // normal interpolation
  cout << "HQenergy:" << HQenergy << "  " << "delta_HQener:" << delta_HQener << "  " << " HQenergy_num:" << HQenergy_num << endl;
